...
do
    ...
    if sqloutput=$(pg_dump -U $TARGET_DATABASE_USER -h $TARGET_DATABASE_HOST -p $TARGET_DATABASE_PORT -d $CURRENT_DATABASE -Fp 2>&1 > /tmp/$DUMP)
    then
        ...
        then
            ...
            then
                ...
                echo -e "Database backup failed to upload for $CURRENT_DATABASE at $(date +'%d-%m-%Y %H:%M:%S'). Error: $gcpoutput" | tee -a /tmp/kubernetes-s3-postgres-backup.log
                has_failed=true
            fi
